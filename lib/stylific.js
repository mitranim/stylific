!function () {
    ...
    document.addEventListener('click', function (event) {
        ...
        if (!(elem instanceof HTMLElement) || hasAttr(elem, 'disabled'))
            return;
        ...
        if (hasAttr(elem, 'data-sf-toggle')) {
            ...
            if (hasClass(parent_1, 'sf-collapse'))
                toggleElem(elem);
            else if (hasClass(parent_1, 'sf-navbar'))
                toggleElem(elem);
            else if (hasClass(parent_1, 'sf-tabset-head')) {
                var tabset = parent_1.parentElement;
    ...
    document.addEventListener('focus', function (event) {
        ...
        if (elem instanceof HTMLElement) {
            var parent_2 = elem.parentElement;
            if (parent_2 && hasAttr(parent_2, 'data-sf-trigger') && ~getAttr(parent_2, 'data-sf-trigger').indexOf('focus')) {
                parent_2.classList.add('sf-tooltip-visible');
            }
        }
    }, true);
    ...
    document.addEventListener('blur', function (event) {
        var elem = event.target;
        if (elem instanceof HTMLElement) {
            var parent_3 = elem.parentElement;
            if (parent_3 && hasAttr(parent_3, 'data-sf-trigger') && ~getAttr(parent_3, 'data-sf-trigger').indexOf('focus')) {
                parent_3.classList.remove('sf-tooltip-visible');
            }
        }
    }, true);
